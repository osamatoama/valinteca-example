...
class Browser:
    ...
    def visit_page(self):
        ...
        self.driver.get('https://www.midasbuy.com/midasbuy/tr/buy/undawngl')
    ...
    def switch_player_id(self, player_id, server_group,server_name):
        try:
        ...
        if original_player_id=='/':
            status = self.driver.execute_script(f"document.querySelector('div.UserTabBox_arrow_right__w5mZD').click();return 'clicked player switch button';")
        else:
        ...
        self.select_server(server_group,server_name)
    ...
    def select_server(self, server_group,server_name):
        print('select_server')
        self.driver.find_element(By.XPATH, '//div[@class="BindLoginPop_select_area__XJT-W"]/i').click()
        self.driver.execute_script('arguments[0].click();', self.driver.find_element(By.XPATH, f'//div[text()="{server_group}"]'))
        time.sleep(1)
        self.driver.execute_script('arguments[0].click();', self.driver.find_element(By.XPATH, f'//div[text()="{server_name}"]'))

    def filter_razer_gold(self):
        print('filter_razer_gold')

        WebDriverWait(self.driver, 10).until(EC.presence_of_element_located((By.XPATH,'//div[@class="FilterIndexPc_filter_box__mqRWq undefined null"]/div[2]'))).click()
